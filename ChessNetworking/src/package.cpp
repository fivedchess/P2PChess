...
namespace Chess {
  ...
  Package::Package(Connectable* from, std::unordered_set<unRouter, unRouter_hash>::iterator to, boost::shared_ptr<std::map<std::string, std::string>> data) : from(from), to(boost::move(to)), data(data) {}
};
